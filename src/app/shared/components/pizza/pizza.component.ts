...
export class PizzaComponent implements OnInit {
  ...
  load() {
    ...
    if (this.chartTittleType == 'Mortes por COVID no Brasil') {

      if (this.pizzaData != undefined) {
        this.pizzaData.forEach(element => {
          this.ChartLabels.push(element.state);
          this.ChartData.push(element.deaths);
        });
      }
    }
        if (this.chartTittleType == 'Casos confirmados no mundo') {

      if (this.paisesData != undefined) {
